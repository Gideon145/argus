...
export const alphaAgent = {
  ...
  fallbackAnalyze(req: QueryRequest): Verdict {
    ...
    // Smarter heuristics (not random checksums)
    const hexBody = address.slice(2);
    
    // Check for address poisoning patterns (repeating chars matching known prefixes)
    if (/^[a-f0-9]{4}[a-f0-9]\1{10,}/i.test(hexBody)) { flags.push('Address poisoning pattern — mimics known prefix'); riskScore += 30; }
    // Check for all-zero or all-f patterns (burn/dead addresses)
    if (/^0{8,}/.test(hexBody) || /^f{8,}/.test(hexBody)) { flags.push('Null/burn address pattern — likely not a real contract'); riskScore += 5; }
    // Very low entropy (autogenerated/mass-deployed)
    const uniqueChars = new Set(hexBody.slice(0, 20).split('')).size;
    if (uniqueChars <= 6) { flags.push('Low entropy — mass-deployed or autogenerated contract'); riskScore += 20; }
    // Numeric-heavy (scam contract generators)
    const digitCount = hexBody.slice(0, 20).split('').filter((c: string) => '0123456789'.includes(c)).length;
    if (digitCount > 14) { flags.push('Numeric-heavy address — typical of scam contract generators'); riskScore += 25; }
