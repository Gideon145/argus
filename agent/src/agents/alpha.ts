...
export const alphaAgent = {
  ...
  fallbackAnalyze(req: QueryRequest, contractData?: ContractData): Verdict {
    ...
    if (contractData?.isContract) {
      ...
      // Address heuristics — on-chain contracts can still be autogenerated/vanity patterns
      const hexBodyA = address.slice(2);
      const uniqueCharsA = new Set(hexBodyA.slice(0, 20).split('')).size;
      const digitCountA = hexBodyA.slice(0, 20).split('').filter((c: string) => '0123456789'.includes(c)).length;
      const digitRunA = hexBodyA.replace(/[a-f]/g, '');
      if (uniqueCharsA <= 6) { f.push('Low entropy address prefix — likely auto-generated'); riskScore += 20; }
      if (digitCountA > 14) { f.push('Numeric-heavy address — common in factory-deployed contracts'); riskScore += 25; }
      if (/(012345|123456|234567|345678|456789|567890|678901|789012|890123|901234)/.test(digitRunA)) { f.push('Sequential hex address — autogenerated pattern'); riskScore += 25; }
    ...
    if (/(012345|123456|234567|345678|456789|567890|678901|789012|890123|901234)/.test(hexBody.replace(/[a-f]/g, ''))) { flags.push('Sequential hex address — autogenerated pattern'); riskScore += 25; numericScore += 25; }
