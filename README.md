...
|---|---|
| **Agentic Sophistication (30%)** | 3 independent agents — DeepSeek-V3, Claude Sonnet 4.5, deterministic rule engine — vote with real stakes. Dissenters pay winners. Each agent has its own Circle SCA wallet and on-chain ELO reputation. The patrol loop runs every 15 min with zero human input: agents scan, stake, settle, and record on-chain entirely on their own. Agents decide, not automate. |
