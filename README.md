...
**??e?? ?f?a?�??. ??a ???s??.** � Three eyes. One verdict.
...
> **5/5 Circle primitives. Agent-to-agent nanopayments. Autonomous patrol loop.** Every piece of the Arc stack, integrated and live. Three autonomous agents with real wallets, real stakes, real ELO � scanning, paying, and patrolling. All on-chain. All verifiable. [Watch them work ?](https://argusarc.dev/patrol)
...
> Agents scan, stake, and settle on their own � every 15 minutes, no human in the loop. Same 3-agent consensus pipeline as a user scan, initiated autonomously. [See it live ?](https://argusarc.dev/patrol)
...
**Live patrol feed:** [argusarc.dev/patrol](https://argusarc.dev/patrol) � every autonomous scan with verdict, consensus breakdown, agent payments, and ArcScan TX link. The agents are provably alive. This is not a demo � it's a living, breathing agent economy.
...
|-------|-------|------|---------------|
| **Agent a** | DeepSeek-V3 | Contract-level security: ownership, proxies, honeypots, access control, upgradeability, external calls | ~$0.001 |
| **Agent �** | Claude Sonnet 4.5 | Tokenomics & market structure: holder distribution, whale concentration, LP depth, trading patterns, buy/sell taxes, wash trading. Tokenomics inferred from training data � live holder/LP queries planned for v0.15 | ~$0.002 |
| **Agent ?** | Rule Engine (local) | Deterministic safety net � reproducible, zero-hallucination checks: address entropy, digit-run heuristics, known scam deployer patterns, EIP-55 validation, blacklist matching. Catches what LLMs miss. | $0 (no API) |

Each council member operates independently � no shared state, no prompt leakage between models. Agent ? is deterministic and reproducible; run the same address twice, get the same result. Agents a and � draw from different model families � DeepSeek-V3 and Anthropic Claude Sonnet 4.5 � with � defaulting to Claude and falling back to DeepSeek for reliability. This creates genuine cognitive diversity across the Council.

**Why a council?** Because single-model security scanners trust one API's opinion. The Council requires agreement to surface truth. When Agent � (the tokenomics skeptic) and Agent ? (the pattern matcher) both flag a contract as RISKY while Agent a calls it SAFE, the Council surfaces the split � and the user sees exactly why each member voted the way it did, with full reasoning.
...
```
Agent ? votes SAFE. Agents a and � vote RISKY.
Consensus: 2/3 ? RISKY. Agent ? is the loser.

Agent ? ? Agent a: 0.0005 USDC ?
Agent ? ? Agent �: 0.0005 USDC ?
...
> **Why 20 USDC per agent?** Agents are judges, not banks. Each holds a flat 20 USDC stake — enough to cover 40,000 dissents at 0.0005 USDC each. Their balances stay stable because stakes are tiny by design. The real money flows elsewhere: the **treasury** holds $16.68+ USDC from accumulated scan fees. Two pools, two purposes — agents hold collateral for verdicts, the treasury holds revenue from scans. Both verifiable on-chain.
>
> | Pool | Balance | Purpose | ArcScan |
> |------|---------|---------|---------|
> | Agent wallets (3) | 20 USDC each | Staking collateral | α · β · γ |
> | Treasury | $16.68+ USDC | Scan fee accumulation | [View](https://testnet.arcscan.app/address/0x0699a029e2e05EC88d6418EC744232702Cf77d81) |
...
# Current stats (1,504 scans, 89.6% consensus)
...
curl https://argus-web-backend-production.up.railway.app/balance/unified/0x0699a029e2e05EC88d6418EC744232702Cf77d81 | jq .
```
...
| Agent ? (SCA) | `0x1fa79f59abbada269de477b45ded38c75a6146de` | [View](https://testnet.arcscan.app/address/0x1fa79f59abbada269de477b45ded38c75a6146de) |
...
| **v0.11** | Telegram bot ([t.me/Argusarc_bot](https://t.me/Argusarc_bot)) — third surface, multi-platform reach | ? Shipped (Jul 1) |
...
| **v0.16** | Agent � on-chain upgrade � real-time holder queries, DEX liquidity data | Post-mainnet |
...
| **Holder distribution + liquidity are estimated** | Agent � infers tokenomics from training data � it does not query holder snapshots or DEX liquidity pools in real-time. Production upgrade: integrate on-chain balanceOf queries + DexScreener/GeckoTerminal APIs. |
...
*Real usage on Arc testnet. Every number is verifiable on-chain. When Arc mainnet launches, these agents will be live as security infrastructure on the chain — 350+ users and 1,504 scans of battle-testing behind them.*

### Metrics

| Metric | Value | Proof |
|--------|-------|-------|
| **Scans processed** | **1,504** | /stats endpoint — on-chain records |
| **Consensus reached** | **1,347 (89.6%)** | 3-agent pipeline live since Jun 16 |
| **Users** | **350+** | Web · CLI · Telegram · multi-platform |
| **Treasury balance** | $16.68+ USDC | [ArcScan](https://testnet.arcscan.app/address/0x0699a029e2e05EC88d6418EC744232702Cf77d81) |
| **Autonomous patrol scans** | **2,407** | 15-min loop, zero human input since launch |
| **Agent economy volume** | 106 payments | Losers pay winners 0.0005 USDC per dissent |
| **ELO leaderboard** | α 10,560 · β 10,477 · γ 10,184 | `/elo` endpoint — on-chain |
...
| argusarc.dev live | Jun 18�20 | 0 | 45 (team tests) | $0.00 |
...
| SDK shipped to npm | Jul 21 | 300+ | 1,504 + 2,407 patrol | $15.86 |
| **OKX 5.7K sales · Current** | **Aug 3** | **350+** | **1,504 + 2,407 patrol** | **$16.68** |
...
*Most users discover Argus through the [Telegram community](https://t.me/argus_communityy) (80+ members) — a group chat where people share scan results, flag suspicious tokens, and help each other stay safe. The community is the distribution channel. Drop in and see what people are scanning right now.*
...
**350+ users, 1,504 scans. Power users run 20+ scans — when Argus clicks, it becomes part of the workflow. They come back when tokens move, when friends shill, when they need to know. Argus grows from first scan into a habit.**
...
The full debug log is at [ENGINEERING_DEBUG_LOG.md](ENGINEERING_DEBUG_LOG.md) — 6 real bugs encountered and solved.
...
**5,700 sold on OKX AI Marketplace. 241 reviews. 4.94?. All verifiable on-chain. Agent #5047. Live now. This is A2A commerce at scale — agents on X Layer paying agents on Arc for contract security, no humans in the loop.**
...
**[t.me/Argusarc_bot](https://t.me/Argusarc_bot)** � `/scan 0x...` from any chat. Auto-creates a Circle wallet per user, pays $0.01 USDC via Gateway x402, shows full agent reasoning + dissent payments. Three surfaces, one oracle.
...
<p align="center">
  ...
  <a href="https://x.com/Argus_arc">x.com/Argus_arc</a> · <a href="https://argusarc.dev">argusarc.dev</a> · <a href="https://argusarc.xyz">alt: argusarc.xyz</a> · <a href="https://github.com/Gideon145/argus">github.com/Gideon145/argus</a>
</p>
